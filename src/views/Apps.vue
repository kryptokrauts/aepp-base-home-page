...
const pathsToMetadata = paths => (state, getters) => paths
  .map(path => ({
    ...getters['appsMetadata/get'](path),
    url: `${PROTOCOL_DEFAULT}//${path}`,
  }));
...
export default {
  ...
  data: () => ({ PROTOCOL_DEFAULT }),
  computed: mapState({
    aeternityApps: pathsToMetadata(aeternityAppsPaths),
    bookmarkedApps: pathsToMetadata(['example.com']),
  }),
};
</script>
